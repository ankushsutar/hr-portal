...
export const MyAttendance = () => {
  ...
  if (isLoadingAttendance || isLoadingRequests) {
    return <div className="p-8 text-slate-500 font-mono text-xs">Loading attendance console...</div>
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-[28px] font-bold text-slate-100 leading-tight tracking-tight">My Attendance</h1>
          <p className="text-xs text-slate-400 font-mono mt-1">PERSONAL PUNCH HISTORY & ANOMALY REQUESTS</p>
        </div>
        <div className="flex items-center gap-2">
          <button 
            onClick={() => setRequestModal({ type: 'WFH', open: true })}
            className="flex items-center gap-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 px-3 py-1.5 rounded text-xs font-medium transition-colors"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="p-5">
          <p className="text-xs font-mono font-medium text-slate-400 uppercase tracking-wider">Present Days</p>
          <h2 className="text-3xl font-mono font-bold text-slate-100 mt-1">18</h2>
        </Card>
        <Card className="p-5">
          <p className="text-xs font-mono font-medium text-slate-400 uppercase tracking-wider">Late Marks</p>
          <h2 className="text-3xl font-mono font-bold text-amber-400 mt-1">2</h2>
        </Card>
        <Card className="p-5">
          <p className="text-xs font-mono font-medium text-slate-400 uppercase tracking-wider">Comp-off Balance</p>
          <h2 className="text-3xl font-mono font-bold text-blue-400 mt-1">1.5</h2>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="p-0 overflow-hidden">
          <div className="px-5 py-3 border-b border-slate-800 bg-slate-900/60">
            <h3 className="font-semibold text-slate-100 text-sm">Recent Punches</h3>
          </div>
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-slate-800 bg-slate-900/40">
                <th className="px-5 py-2.5 text-xs font-mono text-slate-400 uppercase">Date</th>
                <th className="px-5 py-2.5 text-xs font-mono text-slate-400 uppercase">In/Out</th>
                <th className="px-5 py-2.5 text-xs font-mono text-slate-400 uppercase">Status</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/60 text-xs">
              {myLogs.filter(Boolean).map((log: any, idx: number) => (
                <tr key={idx} className="hover:bg-slate-800/40 transition-colors">
                  <td className="px-5 py-3 font-mono text-slate-200">{log.date}</td>
                  <td className="px-5 py-3 text-slate-300">
                    <div className="font-mono text-blue-300">{log.first_in} - {log.last_out}</div>
                    <div className="text-[11px] text-slate-500 mt-0.5 flex items-center gap-1"><MapPin size={11}/> HQ Office</div>
                  </td>
        ...
        <Card className="p-0 overflow-hidden">
          <div className="px-5 py-3 border-b border-slate-800 bg-slate-900/60">
            <h3 className="font-semibold text-slate-100 text-sm">My Requests</h3>
          </div>
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-slate-800 bg-slate-900/40">
            ...
            <tbody className="divide-y divide-slate-800/60 text-xs">
              {myRequests.map((req: any, idx: number) => (
                <tr key={idx} className="hover:bg-slate-800/40 transition-colors">
                  <td className="px-5 py-3">
                    <div className="font-mono font-semibold text-slate-200">{req.type}</div>
                    <div className="text-[11px] text-slate-400 mt-0.5 truncate max-w-[160px]" title={req.reason}>{req.reason}</div>
                  </td>
                  <td className="px-5 py-3 font-mono text-slate-300">{req.date}</td>
                  <td className="px-5 py-3">
              ...
              {myRequests.length === 0 && (
                <tr><td colSpan={3} className="p-6 text-center text-slate-500 font-mono text-xs">No requests logged.</td></tr>
              )}
            </tbody>
          </table>
        </Card>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
          <div className="bg-[#111827] rounded-lg border border-slate-800 shadow-2xl w-full max-w-md overflow-hidden animate-fade-in">
            <div className="px-5 py-3 border-b border-slate-800 flex items-center justify-between bg-slate-900/60">
              ...
              <button onClick={() => setRequestModal({ type: '', open: false })} className="text-slate-400 hover:text-slate-200">
                <XCircle size={18} />
              </button>
            </div>
            <div className="p-5 space-y-4 text-xs">
              ...
              <div>
                <label className="block font-mono text-slate-400 mb-1">Reason / Justification</label>
                <textarea 
